...
with open('optdigits.tes', 'r') as testingFile:
  ...
  accuracy = 0
  tn = len(dbTest) #tn mean total number of the data set.
  ...
  for i,testSample in enumerate(dbTest):
    ...
    predicting_label = classVotes[i].index(max(classVotes[i]))
    if true_label == predicting_label:
        cor +=1
    accuracy = cor/tn
  ...
  for data in dbTest:
      true_label = data[-1]
      predicting_label = clf.predict([data[:-1]])[0]
      if true_label == predicting_label:
          cor +=1
